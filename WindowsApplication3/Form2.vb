...
Public Class Form2
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        ...
        If (Not FolderBrowserDialog2.SelectedPath = "" And Not FolderBrowserDialog1.SelectedPath = "") Then
            Dim files As ReadOnlyCollection(Of String)
            files = My.Computer.FileSystem.GetFiles(FolderBrowserDialog1.SelectedPath, FileIO.SearchOption.SearchAllSubDirectories, "*.vox")
            If (files.Count = 0) Then
                MsgBox("該目錄下無任何vox檔案！", MsgBoxStyle.Critical)
                Return
            End If
            ...
            For i = 0 To files.Count - 1
                ...
                TRACK8_START.Clear()
                Dim value As String = ""
                Dim MySF As IO.StreamReader = New IO.StreamReader(files(i), System.Text.Encoding.Default)
                ...
                savefilename = files(i).Replace(FolderBrowserDialog1.SelectedPath & "\", "")
                savefilename = savefilename.Replace(".vox", ".ksh")

                Label1.Text = savefilename
                create_chart(FolderBrowserDialog2.SelectedPath & "\" & savefilename)
        ...
        End If
        Label1.Text = ""
        MsgBox("批次處理完成！", MsgBoxStyle.Information)
    ...
    Public Sub create_chart(filename As String)
...
"pfiltergain=50" & vbCrLf & "filtertype=peak" & vbCrLf & "chokkakuvol=50" & vbCrLf & "pfilterdelay=0" & vbCrLf & "ver=133" & vbCrLf & "--" & vbCrLf
        'Show Data
        ...
        For k = 0 To end_measure(0)
            ...
            For l = 0 To current_beat(0) - 1
                For m = 0 To 192 / current_beat(1) - 1
                    ...
                    If (Track_Data(0, k, l, m, 0) > 0 Or Track_Data(0, k, l, m, 1) > 0) Then
                        ...
                        If (Track_Data(0, k, l, m, 0) - current_laser(0) = 1 Or Track_Data(0, k, l, m, 0) - current_laser(0) = -1) Then
                            push_string(8) = Laser_convert(Int(current_laser(0) / 127 * 51 + 0.5))
                        Else
                            push_string(8) = Laser_convert(Int(Track_Data(0, k, l, m, 0) / 127 * 51 + 0.5))
                        End If
                    ...
                    If (Track_Data(7, k, l, m, 0) > 0 Or Track_Data(7, k, l, m, 1) > 0) Then
                        ...
                        If (Track_Data(7, k, l, m, 0) - current_laser(1) = 1 Or Track_Data(7, k, l, m, 0) - current_laser(1) = -1) Then
                            push_string(9) = Laser_convert(Int(current_laser(1) * 51 / 127 + 0.5))
                        Else
                            push_string(9) = Laser_convert(Int(Track_Data(7, k, l, m, 0) * 51 / 127 + 0.5))
                        End If
            ...
            output_data = output_data & "--" & vbCrLf
            My.Computer.FileSystem.WriteAllText(filename, output_data, True)
            output_data = ""
        Next
    End Sub
